...
\begin{abstract}
Steady flows refer to packet flows whose packet arrival rates remain relatively steady across multiple consecutive time windows. Such flows represent sustained data transmission behaviors of network applications, and their detection is valuable for bandwidth management, resource planning, and cache optimization. Existing methods require storing per-flow information across multiple time windows to track flow stability, resulting in high memory overhead and low throughput. To address these issues, we propose an efficient and accurate method for detecting steady flows in network traffic called SF-Detector, which separates the filtering and recording functions of packet flows, enabling efficient detection and reporting of steady flows. In particular, SF-Detector consists of a filtering part that employs Three Counting Bloom Filter arrays to exclude non-continuous flows, and a recording part that adopts a hash table to track and verify steady flows. As for the recording part, we define a simple flow stability metric as the ratio of the maximum of packet arrival frequencies across consecutive time windows to their minimum, to efficiently determine steady flows. Additionally, we also design a flow replacement strategy to accommodate each new potentially-steady flow, by evicting the flow with the minimum stability counter in its mapped bucket if the bucket is already full. Furthermore, we provide theoretical error bounds for the frequency estimation in the filtering part and the steady flow detection in the recording part. Finally, We validate the performance of our proposed SF-Detector through extensive experiments on three real-world datasets. Experimental results indicate that SF-Detector achieves a precision of 97\%, a recall rate of 99\%, and a throughput of 40 MIPS, outperforming state-of-the-art methods.
\end{abstract}

\begin{keyword}
Network traffic measurement \sep Steady flows \sep Three Counting Bloom Filter arrays \sep Flow stability metric \sep Flow replacement strategy
\end{keyword}
...
To address the aforementioned problems, we propose SF-Detector, an efficient and accurate method for detecting steady flows in network traffic, which consists of two main components: a filtering part and a recording part. Specifically, the filtering part employs Three Counting Bloom Filter arrays to track packet arrival frequencies across three consecutive time windows, effectively filtering out non-continuous flows before they enter the recording part. This results in only a small number of potentially steady flows entering the recording part, significantly reducing memory overhead. The recording part is implemented as a hash table, where each bucket contains multiple cells to store essential information of potentially steady flows, including flow identifier, current frequency, maximum and minimum frequencies, and steady count. With this information, SF-Detector reports steady flows at the end of each time window based on a stability criterion that compares maximum and minimum frequencies with a tolerance parameter, avoiding complex variance calculations. Additionally, we devise a flow replacement mechanism based on steady count to evict flows with the fewest consecutive steady periods when memory is limited, ensuring efficient memory utilization. The main contributions of this paper are summarized as follows:
\begin{itemize}
    \item Proposing a novel detection method of steady flows called SF-Detector, which adopts a filtering part to exclude non-continuous flows via three counting bloom filter arrays, and a recording part to track and verify potentially steady flows, achieving high accuracy and low memory overhead.
    \item Devising a stability determination method based on extreme value differences, which evaluates flow stability by comparing the ratio of maximum to minimum frequencies across consecutive windows with a tolerance parameter, avoiding complex variance calculations.
    \item Designing a replacement mechanism based on steady count, which replaces the flow with the smallest steady count in a full bucket with a newly arrived potentially steady flow, enhancing the detection accuracy of steady flows.
    \item Deriving theoretical error bounds for the frequency estimation in the filtering part and the steady flow detection in the recording part, demonstrating the strong ability of SF-Detector to accurately detect steady flows.
\end{itemize}

The remainder of this paper is organized as follows. Section 2 defines steady flows and discusses related work in network measurement. Section 3 provides a detailed description of the SF-Detector algorithm and presents a theoretical analysis of the error bounds. Section 4 describes the experimental setup and results, including comparisons with existing solutions. Finally, Section 5 concludes the paper.
...
Considering that non-continuous flows account for the majority of packet flows in network traffic, we construct the framework of SF-Detector in Figure \ref{fig:framework}, whose key idea is to separate the filtering and recording functions of steady flows. The SF-Detector consists of a filtering part and a recording part. The filtering part employs Three Counting Bloom Filter arrays to track flow frequencies across three consecutive time windows and filter out non-continuous flows. We propose a rolling update mechanism for the filtering part to efficiently maintain flow frequency information with minimal memory overhead. The recording part adopts a hash-table-based structure to precisely track and verify potentially steady flows. We design a steady-count-based replacement strategy for the recording part to accommodate new potentially-steady flows when buckets are full, improving the accuracy of steady-flow detection. At the end of each time window, we report steady flows and clear out outdated information and flows that are no longer steady.
...
As for an arrived packet within a flow $f$, we first look up the flow $f$ in the recording part. If the flow $f$ has been recorded in the recording part, we update its frequency in the current time window. Otherwise, we insert the packet into the filtering part to further observe whether the flow $f$ is a potentially steady flow. If its frequencies across three consecutive time windows satisfy the stability condition, we consider the flow $f$ as a potentially steady flow and insert it into the recording part. At the end of each time window, we report the information of steady flows whose $Steady\_count$ reaches the threshold $\Gamma$, and adopt an information cleanup mechanism to clear out flows that no longer satisfy the stability condition. Finally, we perform rolling updates on the counting Bloom filters in the filtering part and reset all flow frequencies in the recording part.

\subsection{Filtering Part}
The filtering part employs Three Counting Bloom Filter arrays to track flow frequencies across three consecutive time windows and filter out non-continuous flows. This design enables accurate tracking of frequency variation patterns within consecutive time windows, all within a fixed storage footprint.

\textbf{The data structure of the filtering part:} As shown in Figure \ref{fig:filter}, the filtering part comprises three arrays $BF_1$, $BF_2$, and $BF_3$, each containing $w$ counters. These are respectively associated with $k$ mutually independent hash functions $H_1(\cdot), H_2(\cdot), ..., H_k(\cdot)$, with the counters recording the arrival frequency of the flow within each time window. $BF_1$ records the frequency of flow occurrences in the earliest time window, $BF_2$ tracks occurrences in intermediate windows, while $BF_3$ monitors the current window's frequency in real time.
...
\textbf{Packet processing:} When the system receives a new flow $f$ not yet recorded in the recording part, it maps $f$ through $k$ independent hash functions $H_1(f), ..., H_k(f)$ to $k$ specific positions $BF_3[H_i(f)]$ within $BF_3$ (where $0 \le H_i(f) \le w-1$, $1 \le i \le k$), incrementing the counter values at these positions by 1.

\textbf{The detection of potentially steady flows:} At the end of each time window, for flows under detection, the system queries the counter values at corresponding hash positions across the three counting Bloom filters. There are two cases:
\begin{itemize}
    \item \textbf{Case 1:} If the counter values of $BF_1[H_i(f)]$, $BF_2[H_i(f)]$, and $BF_3[H_i(f)]$ are all non-zero and satisfy the stability condition, then flow $f$ is deemed to have arrived continuously over three consecutive time windows and is transferred to the recording part as a potentially steady flow.
...
1) To check $f_1$, we obtain its mapped counters in the three Bloom filters by hashing. The estimated frequencies across three windows are 5, 4, and 5 respectively. All counters are non-zero and satisfy the stability condition. Thus, $f_1$ is detected as a potentially steady flow and transferred to the recording part.
...
The recording part employs a hash-table-based structure to precisely track and verify potentially steady flows. This design avoids complex variance calculations by recording each flow's maximum and minimum values, while introducing a steady-count metric to track the flow's sustained stability.
...
\textbf{Packet processing:} As for an arrived packet within a flow $f$, we first look up the flow $f$ in the recording part. If the flow $f$ is recorded in the recording part, we increment its frequency $F_{cur}$ by 1 and update $Max\_val$ and $Min\_val$ accordingly. Otherwise, we insert the packet into the filtering part.

\textbf{The insertion of a potentially steady flow:} If the flow $f$ is detected as a potentially steady flow by the filtering part, we need to look up the recording part for an empty cell in bucket $A[h(f)]$ and place the flow $f$ into it. There are two cases:
\begin{itemize}
    \item \textbf{Case 1:} There is an empty cell in the bucket $A[h(f)]$. In this case, we insert the flow $f$ into the empty cell. Specifically, we set Flow ID as $f$, $F_{cur}$ as 0, $Max\_val$ as the maximum of the estimated frequencies across three windows from the filtering part, $Min\_val$ as the minimum of these frequencies, and $Steady\_count$ as 0.
...
\textbf{The report of steady flows:} At the end of each time window, we traverse each cell in the recording part to report current steady flows. For each non-empty cell, we check if the stability condition is satisfied and whether $Steady\_count$ reaches the threshold $\Gamma$. There are three cases:
\begin{itemize}
...
\textbf{A running example:} Figure \ref{fig:record_table} depicts a running example of the recording part. Assume that the fluctuation tolerance $\beta = 0.2$, the threshold of consecutive stable windows $\Gamma = 3$, and the current time window $W_{cur} = 5$. The insertion of $f_1$, $f_2$, $f_3$ and the end-of-window processing can be described as follows.
...
3) To insert $f_3$ detected as a potentially steady flow, there is no empty cell in the bucket $A[h(f_3)]$. We select the flow with the smallest $Steady\_count$ in the bucket and replace it with $(f_3, 0, 12, 12, 0)$.
...
In this section, we perform the theoretical analysis of SF-Detector. We first derive the error bounds of flow frequency estimation in the filtering part in Section 3.4.1. Then, we infer the error bounds of steady flow detection in the recording part in Section 3.4.2. Finally, we provide the space and time complexities of SF-Detector in Section 3.4.3.
...
Since all $k$ hash functions are independent, the probability that all mapped counters exceed the error threshold equals the product of individual probabilities. Consequently, we can derive the error bound of flow frequency estimation in the filtering part as
\begin{equation}
    P(\hat{f} - f > \delta \cdot W) \le \left(\frac{1}{\delta \cdot w}\right)^k.
\end{equation}
\hfill $\square$

\subsubsection{The error bounds of steady flow detection in the recording part}

\begin{theorem}
Suppose the recording part consists of $M$ buckets, each of which contains $n$ cells. The recording part records $F$ flows, where there are $S$ flows whose $Steady\_count$ values are lower than the threshold $\Gamma$. Let $f$ and $\hat{f}$ respectively be the true frequency and estimated one of a flow in the recording part. We can infer the error bound of flow frequency estimation in the recording part as
\begin{equation}
    E\left(|\hat{f} - f|\right) < \frac{1}{M} \cdot \left(\frac{e^{-\lambda} \lambda^n}{n!}\right)^3 \cdot \frac{S}{F \cdot n}.
\end{equation}
\end{theorem}

\textit{Proof.} We first analyze the insertion process in the recording part:

\textbf{I:} As for any flow $g$, if it has been recorded in the recording part, we directly increase its counter value by 1. In this case, the estimated frequency of flow $g$ is equal to its true frequency.

\textbf{II:} If the flow $g$ is not recorded in the recording part and there is an empty cell in its directly mapped bucket, the flow $g$ is inserted into this empty cell. In this case, the estimated frequency of flow $g$ is also equal to its true frequency.
...
The flow $g$ is replaced because its $Steady\_count$ is the smallest. Consequently, we can derive the error bound of flow frequency estimation in the recording part as
\begin{equation}
...
\textbf{Space complexity:} SF-Detector consists of a filtering part and a recording part. The filtering part contains $3 \times w$ counters, where $w$ is the number of counters in each Counting Bloom Filter. Meanwhile, the recording part contains $M \cdot n$ cells, where $M$ is the number of buckets and $n$ is the number of cells for each bucket. Suppose the size of each counter in the filtering part and each cell in the recording part respectively as $S_1$ and $S_2$, we can get the memory size of SF-Detector as $3 \cdot w \cdot S_1 + M \cdot n \cdot S_2$. In our SF-Detector, each counter in the filtering part is set as 8 bits. Each cell in the recording part consists of five fields: Flow ID, $F_{cur}$, $Max\_val$, $Min\_val$, and $Steady\_count$. Flow ID is usually manifested as a flow fingerprint typically with 32 bits. For $F_{cur}$, $Max\_val$, and $Min\_val$, 12 bits are adequate to record the frequency of a flow in a time window with 100K packets. $Steady\_count$ can be configured with 8 bits to record the number of consecutive stable windows. In summary, $S_1$ is 8 bits and $S_2$ is 76 bits. According to our experimental settings, $w$, $M$, and $n$ are typically set to 4096, 1024, and 4, respectively. Consequently, we can calculate the memory size of SF-Detector approximately as 51KB. In conclusion, SF-Detector is a lightweight method with low space complexity.

\textbf{Time complexity:} For an arrived packet within a flow $f$, we first look up the flow $f$ in the recording part by hashing. This lookup process involves $n$ cells, where $n$ is the number of cells in each bucket of the recording part. If the flow $f$ has been recorded in the recording part, we only need to update its frequency in the current time window. This case is executed with the time complexity $O(n)$. Otherwise, we need to map the flow into the filtering part using its $k$ hash functions and update the $k$ mapped counters. If the updated counter does not satisfy the stability condition, the flow is not yet a potentially steady flow, and packet processing ends. This case performs with the time complexity $O(n + k)$. If the flow is detected as potentially steady, we insert the flow $f$ into the recording part, which requires checking $n$ cells. Hence, the execution of this case needs to take the time complexity $O(2n + k)$. In summary, SF-Detector processes each packet with the time complexity $O(2n + k)$ in the worst case. Since $n$ and $k$ are generally configured as very small values (typically $n = 4$ and $k = 3$), SF-Detector is a lightweight method with very low time complexity.
...
\textbf{Datasets:}
\begin{itemize}
    \item \textbf{CAIDA Dataset:} The CAIDA dataset \cite{b30} was collected by the University of California, San Diego (UCSD). We select 10M packets from CAIDA2020 and CAIDA2022 traces respectively, which contains 1,245 steady flows for the time window size set as 1,500 packets.
    \item \textbf{MAWI Dataset:} The MAWI dataset \cite{b31} was collected from daily traces at the transit link of WIDE to the upstream ISP. We select 10M packets, which contains 1,102 steady flows for the time window size set as 1,500 packets.
\end{itemize}

\textbf{Implementation:}
We implement the state-of-the-art methods including SF-Detector, SteadySketch \cite{b23}, Strawman, SteadySketchLite, and SteadySketchEnhanced, and integrate them in a C++ program. For SteadySketch, we set the ratio of the memory usage of its RBF (Rolling Bloom Filter) to its total memory size as 0.5. Meanwhile, its counter array contains slots determined by given memory size. For Strawman, we implement a basic hash-table-based method without filtering. For SteadySketchLite, we use a simplified version of SteadySketch with reduced hash functions. For SteadySketchEnhanced, we extend SteadySketch with additional stability verification. For SF-Detector, we set the number of counters in its filtering part to 4096 and 4 cells for each bucket in its recording part, where the number of buckets is determined by given memory size. We set the fluctuation tolerance threshold $\beta$ as 0.2, the number threshold of consecutively stable time windows $\Gamma$ as 3, and the time window size as 1,500 packets. Subsequently, we run the program on a server configured with an Intel Core i7 processor and 16GB of memory.
